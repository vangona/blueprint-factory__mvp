...
function LongtermParent({ userObj, parent }) {
  ...
  const getTime = () => {
    if (parent.daedline) {
      ...
      const deadlineTimeStr = `${Year}-${Month > 9 ? Month : `0${Month}`}-${
  ...
  return (
    <Container>
      ...
      <Content>
        <Explain>{parent && parent.explain}</Explain>
        <Deadline>{deadlineTime && `${deadlineTime}까지`}</Deadline>
      </Content>
